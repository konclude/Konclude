...
#include "CReasonerEvaluationGroupRequestReasonerMemoryUsageAnalyser.h"
...
namespace Konclude {
	...
	namespace Test {
		...
		namespace Evaluation {
			...
			CReasonerEvaluationGroupRequestReasonerMemoryUsageAnalyser::CReasonerEvaluationGroupRequestReasonerMemoryUsageAnalyser() {
			}
			...
			bool CReasonerEvaluationGroupRequestReasonerMemoryUsageAnalyser::analyseGroupedEvaluationData(const QStringList& groubList, const QStringList& requestList, const QStringList& reasonerList, const QString& outputDirectory, CReasonerEvaluationAnalyseContext* context, CReasonerEvaluationGroupRequestSelector* selector) {
				...
				QString outputFileString(outputDirectory+mOutputFileName+"memory-usage");
							...
							if (selector->isRequestSelected(groupString,requestString)) {
								...
								if (allDataAvailable) {
									...
									foreach (QString reasonerString, reasonerList) {
										...
										if (doubleDataValue) {
											double memoryUsageValue = doubleDataValue->getValue();

											tableMultiFormatOutputWriter.addValuesToLastTableRow(QList<double>()<< memoryUsageValue);

											mean += memoryUsageValue;
											min = qMin(min, memoryUsageValue);
											max = qMax(max, memoryUsageValue);
											count++;
										}
									}
									mean /= (double)count;
					...
					if (context->isGNUPlottingActivated()) {
						...
						foreach (QString plotFileString, reasonerCompGNUPlotFileList) {
							if (true /*plotFileString.contains("-by-Average-")*/) {
								...
								context->getGNUPlotPlotter()->createReasonerComparisonPlot(plotFileString,context->getAdditionalTitleString()+title+"[bytes]",reasonerList,outputFile);
							}
						}
					}
					if (context->isHighchartPlottingActivated()) {
						foreach (QString plotFileString, reasonerCompHighchartPlotFileList) {
							if (true /*plotFileString.contains("-by-Average-")*/) {
								...
								QString title = getPrettyTitleFromOutputFile(outputFile);
								context->getHighchartPlotter()->createReasonerMemoryUsagePlot(plotFileString,context->getAdditionalTitleString()+title+"[bytes]",reasonerList,outputFile,selector->getNameString()+" :: "+context->getAdditionalTitleString());
							}
						}
					}
			...
			QString CReasonerEvaluationGroupRequestReasonerMemoryUsageAnalyser::getPrettyTitleFromOutputFile(const QString& outputFile) {
				...
				title.replace("memory-usage", "Reaoner Max Memory Usage, ");
				title.replace("-", " ");
