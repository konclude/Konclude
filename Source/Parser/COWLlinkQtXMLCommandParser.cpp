...
namespace Konclude {
	...
	namespace Parser {
		...
		COWLlinkQtXMLCommandParser::COWLlinkQtXMLCommandParser() : CLogIdentifier("::Konclude::Parser::OWLlinkQtCommandParser",this) {
		...
		void COWLlinkQtXMLCommandParser::parseOWLlinkRequestNode(QDomElement *requestNode) {
			...
			while (!node.isNull()) {
				...
				if (docElemName == "ox:GetDescription" || docElemName == "GetDescription") {
					...
					preComm = parseLoadKnowledgeBaseOWLXMLOntologyNode(&node);
				...
				if (preComm) {
					...
					if (queryCommand) {
						parseQueryExtensions(&node,queryCommand);
		...
		CLoadKnowledgeBaseOWLXMLOntologyCommand *COWLlinkQtXMLCommandParser::parseLoadKnowledgeBaseOWLXMLOntologyNode(QDomElement *node) {
			QString kbName = node->attribute(QString("kb"));
			...
			return new CLoadKnowledgeBaseOWLXMLOntologyCommand(kbName,ontoIRIList);
		}
		...
		void COWLlinkQtXMLCommandParser::parseQueryExtensions(QDomElement *node, CKnowledgeBaseQueryCommand *queryCommand) {
		...
		bool COWLlinkQtXMLCommandParser::requiresComplexQuery(QDomElement *node) {
