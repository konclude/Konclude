...
#include "CWriteOREFunctionalClassSubsumptionsHierarchyQuery.h"
...
namespace Konclude {
	...
	namespace Reasoner {
		...
		namespace Query {
			...
			CWriteOREFunctionalClassSubsumptionsHierarchyQuery::CWriteOREFunctionalClassSubsumptionsHierarchyQuery(CConcreteOntology *premisingTaxonomyOntology, CConfigurationBase *configuration, const QString& outputFileString, const QString &superClassName, const QString &subClassHierarchyQueryName)
			...
			CQueryResult *CWriteOREFunctionalClassSubsumptionsHierarchyQuery::getQueryResult() {
				return nullptr;
			}

			CQuery* CWriteOREFunctionalClassSubsumptionsHierarchyQuery::addQueryError(CQueryError* queryError) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::forcedPathCreated(const QString& filePath) {
			...
			bool CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeInconsistentHierarchyResult() {
			...
			CQueryResult *CWriteOREFunctionalClassSubsumptionsHierarchyQuery::constructResult(CTaxonomy *taxonomy) {
				mQueryAnswered = true;
			...
			bool CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeSubClassHierarchyResult(CTaxonomy *taxonomy) {
				forcedPathCreated(mOutputFileNameString);
				QFile outputFile(mOutputFileNameString);
				if (outputFile.open(QIODevice::WriteOnly)) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeBottomEquivalences(CHierarchyNode* node, QFile* outputFile) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeTopEquivalences(CHierarchyNode* node, QFile* outputFile) {
				if (node->getEquivalentConceptCount() > 1) {
					if (mWriteDirectEquivalences) {
						writeEquivalenceRelations(node,outputFile);
					} else {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeEquivalenceRelations(const QStringList& classNameList, QFile* outputFile) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeEquivalenceRelations(CHierarchyNode* node, QFile* outputFile) {
				const QStringList& classNameList(node->getEquivalentConceptStringList(mUseAbbreviatedIRIs));
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeEquivalences(CHierarchyNode* node, QFile* outputFile) {
				if (node->getEquivalentConceptCount() > 1) {
					if (mWriteDirectEquivalences) {
						writeEquivalenceRelations(node,outputFile);
					} else {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeDeclarations(CHierarchyNode* node, QFile* outputFile) {
				if (mWriteDeclarations) {
					const QStringList& classNameList(node->getEquivalentConceptStringList(mUseAbbreviatedIRIs));
					foreach (const QString& className, classNameList) {
						writeDeclaration(className,outputFile);
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeDeclaration(const QString& className, QFile* outputFile) {
				QString writeString = QString("Declaration(Class(<%1>))\n").arg(className);
				outputFile->write(writeString.toUtf8());
			}


			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeOntologyStart(QFile* outputFile) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeOntologyEnd(QFile* outputFile) {
				outputFile->write(QString(")").toUtf8());
			}


			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeSubClassRelations(CHierarchyNode* superNode, CHierarchyNode* subNode, QFile* outputFile) {
			...
			void CWriteOREFunctionalClassSubsumptionsHierarchyQuery::writeSubClassRelation(const QString& subClassName, const QString& superClassName, QFile* outputFile) {
			...
			QString CWriteOREFunctionalClassSubsumptionsHierarchyQuery::getQueryName() {
				return mQueryName;
			}

			QString CWriteOREFunctionalClassSubsumptionsHierarchyQuery::getQueryString() {
				return mQueryString;
			}

			bool CWriteOREFunctionalClassSubsumptionsHierarchyQuery::hasAnswer() {
				return mQueryAnswered;
			}

			QString CWriteOREFunctionalClassSubsumptionsHierarchyQuery::getAnswerString() {
				return QString("Subclass Hierarchy written to file '%1'").arg(mOutputFileNameString);
			}


			bool CWriteOREFunctionalClassSubsumptionsHierarchyQuery::hasError() {
