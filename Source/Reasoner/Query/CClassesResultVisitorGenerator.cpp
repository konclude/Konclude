...
namespace Konclude {
	...
	namespace Reasoner {
		...
		namespace Query {
			...
			CClassesResultVisitorGenerator::CClassesResultVisitorGenerator(CClassesResult* classesResult, bool abbreviatedIRIs) {
				mClassesResult = classesResult;
				mAbbreviatedIRIs = abbreviatedIRIs;
			}

			bool CClassesResultVisitorGenerator::visitConcept(CConcept* concept, CConceptRealization* conRealization) {
				...
				if (!conceptString.isEmpty()) {
					mClassesResult->addClass(conceptString);
				}
				return true;
			}
