		...
		namespace Preprocess {
			...
			CConcreteOntology *CMappingUpdaterPreProcess::preprocess(CConcreteOntology *ontology, CPreProcessContext* context) {
				if (ontology) {
					...
					mIndiVec = abox->getIndividualVector();
					...
					mNameRoleHash = ontology->getStringMapping()->getPropertyNameRoleMappingHash();
					mNameIndiHash = ontology->getStringMapping()->getIndividualNameIndividualMappingHash();
					...
					for (qint64 i = rolesStartUpdateInd; i < rolesCounts; ++i) {
						...
						if (role) {
							if (role->hasPropertyName()) {
								mNameRoleHash->insert(CIRIName::getRecentIRIName(role->getPropertyNameLinker()),role);
							}
				...
				qint64 indiStartUpdateInd = mIndiVec->getReferencedItemCount();
				qint64 indiCounts = mIndiVec->getItemCount();
				for (qint64 i = 0; i < indiCounts; ++i) {
					CIndividual *indi = mIndiVec->getLocalData(i);
					if (indi) {
						if (indi->hasIndividualName()) {
							mNameIndiHash->insert(CIRIName::getRecentIRIName(indi->getIndividualNameLinker()),indi);
						}
						if (i < indiStartUpdateInd) {
							CIndividualTermExpression* indiTerm = mIndiIndiTermHash->value(indi,nullptr);
							if (indiTerm) {
								CIndividual* prevIndi = mIndiVec->getReferenceData(i);
								if (prevIndi) {
									indiTerm = mIndiIndiTermHash->value(prevIndi,nullptr);
								}
								if (indiTerm) {
									mIndiIndiTermHash->insert(indi,indiTerm);
									mIndiTermIndiHash->insert(indiTerm,indi);
								}
							}
						}
					}
			...
			bool CMappingUpdaterPreProcess::collectActiveOntologyItemsFromActiveEntities() {
				...
				LOG(INFO,"::Konclude::Reasoner::Preprocess::MappingUpdater",logTr("Found %1 active classes, %2 active object/data properties, %3 active individuals.").arg(activeClassConceptSet->count()).arg(activeRoleSet->count()).arg(activeIndividualSet->count()),this);
