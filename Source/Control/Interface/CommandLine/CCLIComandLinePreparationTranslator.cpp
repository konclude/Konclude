				...
				QStringList CCLIComandLinePreparationTranslator::combineTranslatedArguments() {
					...
					if (!mHelpRequested) {
						...
						if (!mConfigConfList.isEmpty()) {
							translatedArgList += QString("-OWLlinkBatchProcessingLoader +=Konclude.OWLlink.CloseAfterProcessedRequest=false +=Konclude.OWLlink.BlockUntilProcessedRequest=true");
