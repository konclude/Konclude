	...
	namespace Control {
		...
		namespace Interface {
			...
			namespace CommandLine {
				...
				CCLIConsistencyComandLinePreparationTranslator::CCLIConsistencyComandLinePreparationTranslator(CLoaderFactory *loaderFactory) : CCLIComandLinePreparationTranslator(loaderFactory) {
					mOperationTypeString = QString("Consistency");
					mMinNecessaryArguments = true;
					mFirstArgumentTranslation = false;
					mLoaderTypeString = QString("-CLIConsistencyBatchProcessingLoader");
				}


				CCLIConsistencyComandLinePreparationTranslator::~CCLIConsistencyComandLinePreparationTranslator() {
				}

				bool CCLIConsistencyComandLinePreparationTranslator::hasNeccessaryParameters() {
				...
				bool CCLIConsistencyComandLinePreparationTranslator::translate(const QString& argumentText, QStringList& remainingArgList) {
					...
					if (!mSkipTranslation) {
						if (CCommandLinePreparationTranslator::translate(argumentText,remainingArgList)) {
							...
							} else if (modArgText == "consistency" || modArgText == "cons") {
								argumentTranslated = true;
				...
				bool CCLIConsistencyComandLinePreparationTranslator::canTranslate(const QString& string) {
					if (QString::compare(string,"consistency",Qt::CaseInsensitive) == 0 || QString::compare(string,"cons",Qt::CaseInsensitive) == 0) {
						return true;
					}
					return false;
				}
