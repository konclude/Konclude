...
namespace Konclude {
	...
	namespace Control {
		...
		namespace Loader {
			...
			CAnalyseReasonerLoader::CAnalyseReasonerLoader() {
			}
			...
			CLoader *CAnalyseReasonerLoader::init(CLoaderFactory *loaderFactory, CConfiguration *config) {
				...
				if (data) {
					data->readFromString("ASAnalyseReasonerManager");
				}
