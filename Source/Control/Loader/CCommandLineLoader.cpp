...
namespace Konclude {
	...
	namespace Control {
		...
		namespace Loader {
			...
			QStringList CCommandLineLoader::getEncodedArguments(int argc, char *argv[], bool skipFirst) {
				QString argString;
				int startArg = 0;
				if (skipFirst) {
					startArg = 1;
				...
				for (int i = startArg; i < argc; ++i) {
					...
					QString translatedArgString = QString::fromLocal8Bit(argv[i]);
					argString += QString("%1").arg(translatedArgString).trimmed();
				}
