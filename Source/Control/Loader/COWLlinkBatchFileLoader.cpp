		...
		namespace Loader {
				...
				if (!reqFileString.isEmpty()) {
					...
					if (file.open(QIODevice::ReadOnly)) {
						...
						if (mConfLogProcessingTimes) {
							...
							LOG(INFO,getLogDomain(),logTr("Read XML content in %1 milliseconds.").arg(readMilliSeconds),this);
						}
			...
			COWLlinkProcessor *COWLlinkBatchFileLoader::concludeOWLlinkContent() {
				...
				if (mConfLogProcessingTimes) {
					...
					LOG(INFO,getLogDomain(),logTr("OWLlink request processed in %1 milliseconds.").arg(responseMilliSeconds),this);
				}
				if (blockUntilProcessed) {
					blockingSem.release();
				}
