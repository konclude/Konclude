...
namespace Konclude {
	...
	namespace Control {
		...
		namespace Loader {
			...
			CCLIDataPropertyClassificationBatchProcessingLoader::CCLIDataPropertyClassificationBatchProcessingLoader() {
			}
			...
			void CCLIDataPropertyClassificationBatchProcessingLoader::createTestingCommands() {
				createClassificationTestingCommands();
			}


			void CCLIDataPropertyClassificationBatchProcessingLoader::createClassificationTestingCommands() {
				logOutputMessage(QString("Starting data property classification for '%1'.").arg(mRequestFileString));
				...
				if (!mResponseFileString.isEmpty()) {
					CWriteCustomQueryCommand* writeHierarchyCommand = new CWriteCustomQueryCommand(testKB,CWriteQuery::WRITESUBDATAPROPERTYHIERARCHY,new CWriteQueryFileOWL2XMLSerializer(mResponseFileString));
